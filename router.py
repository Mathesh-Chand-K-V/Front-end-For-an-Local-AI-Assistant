...
def clean_code(text):
    skip_patterns = {
        "pip install", "make sure", "this code", "example",
        "below", "output:", "here is", "here's", "certainly",
        "# example", "# this is",
    }
    out = []
...
def _parse(cmd):
    """Split 'cmd | content' → (parts, content). Preserves filename case."""
    if "|" in cmd:
...
def route(cmd):
    ...
    # Only lowercase the command token, preserve case of filenames/content
    ...
    if command == "!read":
        ...
        # Pass just the filename — files.py read() accepts full paths
